      ...
      <div className="bg-white p-6 rounded-[2rem] shadow-sm border border-slate-100 flex flex-col md:flex-row gap-4 items-center">
        <div className="relative flex-1 w-full">
          <input
            ...
            className="w-full pl-12 pr-4 py-3 bg-slate-50 border-none rounded-2xl focus:ring-2 focus:ring-blue-500 transition-all outline-none font-medium text-sm"
            value={filter}
            ...
            <table className="w-full text-left rtl:text-right">
              <thead>
                <tr className="bg-slate-50/50 text-[11px] font-bold text-slate-400 uppercase tracking-[0.15em] border-b border-slate-100">
                  ...
                  <th className="px-8 py-5">Name / Model</th>
                ...
                {filteredAssets.map((asset) => (
                  <tr key={asset.id} className="hover:bg-slate-50/80 transition-colors group">
                    <td className="px-8 py-5">
                      <span className="font-mono text-xs font-bold text-blue-600 bg-blue-50 px-3 py-1.5 rounded-lg border border-blue-100/50">{asset.id}</span>
                    </td>
                    <td className="px-8 py-5">
                      <div>
                        <Link to={`/assets/${asset.id}`} className="font-bold text-slate-900 group-hover:text-blue-600 transition-colors">{asset.name}</Link>
                        <p className="text-[10px] font-bold text-slate-400 uppercase tracking-tighter">{asset.model}</p>
                      </div>
                    </td>
                    <td className="px-8 py-5 text-xs font-bold text-slate-600 uppercase tracking-tight">{asset.type.replace('_', ' ')}</td>
                    <td className="px-8 py-5">{getStatusBadge(asset.status)}</td>
                    <td className="px-8 py-5">
                       <div className="w-32">
